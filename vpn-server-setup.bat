@echo off
echo ========================================
echo   VPN - SERVER SETUP (run once on VPS)
echo ========================================
echo.
...
scp -O "%~dp0server-setup.sh" root@%XRAY_SERVER%:/tmp/server-setup.sh
if %errorlevel% neq 0 (
...
echo.
ssh root@%XRAY_SERVER% "bash /tmp/server-setup.sh"
if %errorlevel% neq 0 (
    echo.
...
echo   2. Run vpn-setup.bat  (router setup)
echo   3. Run vpn-start.bat  (start VPN)
echo ========================================
echo.
pause
