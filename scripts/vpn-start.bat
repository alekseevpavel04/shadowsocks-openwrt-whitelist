...
if "%XRAY_UUID%"=="YOUR_UUID" (echo ERROR: XRAY_UUID is still placeholder in config.bat & pause & exit /b 1)
if "%XRAY_PUBLIC_KEY%"=="" (echo ERROR: XRAY_PUBLIC_KEY not set in config.bat & pause & exit /b 1)
if "%XRAY_PUBLIC_KEY%"=="YOUR_PUBLIC_KEY" (echo ERROR: XRAY_PUBLIC_KEY is still placeholder in config.bat & pause & exit /b 1)
if "%XRAY_SHORT_ID%"=="" (echo ERROR: XRAY_SHORT_ID not set in config.bat. Get it from VPS: cat /usr/local/etc/xray/config.json & pause & exit /b 1)
if "%XRAY_SHORT_ID%"=="YOUR_SHORT_ID" (echo ERROR: XRAY_SHORT_ID is still placeholder in config.bat & pause & exit /b 1)
if "%XRAY_SNI%"=="" (echo ERROR: XRAY_SNI not set in config.bat. Get a free domain at duckdns.org pointing to XRAY_SERVER & pause & exit /b 1)
if "%XRAY_SNI%"=="YOUR_DOMAIN.duckdns.org" (echo ERROR: XRAY_SNI is still placeholder in config.bat & pause & exit /b 1)
...
powershell -Command "$c=(Get-Content '%TEMPLATES_DIR%\xray-router.json') -replace '__SERVER_IP__','%CONNECT_SERVER%' -replace '__UUID__','%XRAY_UUID%' -replace '__PUBLIC_KEY__','%XRAY_PUBLIC_KEY%' -replace '__SHORT_ID__','%XRAY_SHORT_ID%' -replace '__SNI__','%XRAY_SNI%'; [System.IO.File]::WriteAllLines('%TMPCONFIG%',$c)"
...
if not "%REALITY_ERRS%"=="0" (
    ...
    echo       Credentials in config.bat may not match VPS.
    echo       Check: XRAY_UUID, XRAY_PUBLIC_KEY, XRAY_SHORT_ID
    echo.
