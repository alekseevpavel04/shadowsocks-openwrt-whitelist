...
for %%s in (youtube.com discord.com instagram.com x.com tiktok.com twitter.com spotify.com incogniton.com) do (
    curl -so /dev/null -w "  %%s: %%{http_code} | %%{time_total}s" --max-time 10 "https://%%s"
    echo.
)
...
for %%s in (google.com telegram.org t.me telegram.me github.com reddit.com twitch.tv) do (
    curl -so /dev/null -w "  %%s: %%{http_code} | %%{time_total}s" --max-time 10 "https://%%s"
    echo.
)
